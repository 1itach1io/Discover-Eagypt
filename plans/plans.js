...
const app={
  ...
  async downloadPDF(){
    ...
    if(!element){
      alert(this.lang==='ar'?'لا توجد خطة لتحميلها':'No plan to download');
      return;
    }
    ...
    try {
      ...
      // الانتظار قليلاً لتحميل العناصر
      await new Promise(resolve => setTimeout(resolve, 500));
      
      const opt = {
        margin: [10, 10, 10, 10],
        filename: `egypt-trip-${Date.now()}.pdf`,
      ...
      this.hideLoader();
      alert(this.lang==='ar'?'حدث خطأ في تحميل PDF. جرب مرة أخرى.':'Error downloading PDF. Please try again.');
    }
  },
  ...
  savePlan(){
    if(!this.user){
      const msg = this.lang==='ar'?'يجب تسجيل الدخول أولاً':this.lang==='en'?'Please login first':'Veuillez d\'abord vous connecter';
      alert(msg);
      return;
    }
    const planData={
    ...
    db.collection('plans').add(planData)
      .then(()=>{
        const msg = this.lang==='ar'?'تم حفظ الخطة بنجاح':this.lang==='en'?'Plan saved successfully':'Plan enregistré avec succès';
        alert(msg);
      })
      .catch(e=>{
        const msg = this.lang==='ar'?'فشل الحفظ':this.lang==='en'?'Failed to save':'Échec de l\'enregistrement';
        alert(msg);
      });
  },
